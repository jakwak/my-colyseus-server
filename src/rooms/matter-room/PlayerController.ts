  ...
  update() {
    ...
    for (const [sessionId, player] of this.players.entries()) {
      ...
      if (
        ...
        const margin = 40
        let safeX = curX, safeY = curY, dist = 0
        // 경계선(벽) 근처가 아니면 최소 500px 이상 떨어진 곳만 목표로 삼음
        for (let i = 0; i < 10; i++) { // 10회 시도 제한
          ...
          const nearWall =
            safeX < margin + 100 || safeX > SCREEN_WIDTH - margin - 100 ||
            safeY < margin + 100 || safeY > SCREEN_HEIGHT - margin - 100
